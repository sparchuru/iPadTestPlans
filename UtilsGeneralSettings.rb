       ...
       def UtilsGeneralSettings.SelectPortfoliosAndWatchlists(driverEle=nil, pCount, wCount)
          ...
          # end
           ...
           if arrWatchlists.length >= wCount
               if platform == "9.3"
                     xpath_watchlist = AddPortfolioWatchListPagePO.ReplaceString(AddPortfolioWatchListPagePO::ScrollWatchlist_Xpath,"#{wCount}");
                     puts xpath_watchlist
                  driver.execute_script 'mobile: scroll', toVisible: 'true' ,:element => find_element(:xpath,xpath_watchlist ).ref
              else
                  watchlistXpath = AddPortfolioWatchListPagePO.ReplaceString(AddPortfolioWatchListPagePO::Watchlist10x_Xpath,"#{wCount}");
                  driver.execute_script 'mobile: scroll', toVisible: 'true' ,:element => find_element(:xpath,watchlistXpath).ref 
              end
