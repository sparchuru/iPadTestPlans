...
def RuniPhoneSetupWizard6()
  begin
    Log.TestScript("iPhoneSetupWizard6.rb")
    Log.TestCase(128328, "iPhoneSetupWizard6")
...
################################ Test Step  #############################################################
    stepId = 621474
    Log.TestStep(stepId, "No - Today' Top Stories")
      begin
          ...
          question =UtilsMobile.GetElementWithXpath(oPage,question_xpath,20, false)
          if question == false
              Log.Error("Test step #{stepId}:User not navigated to next page after click operation on No  button.")
              Log.Picture("#{UtilsMobile.TakeSimulatorScreenshot(oPage)}", "Screencapture")
              return false
          end
          # Verify the Yes, No and Skip button appeared or not
...
################################ Test Step  #############################################################
    stepId = 621639
    Log.TestStep(stepId, "FactSet Drive")
      begin
          #Tap on the Redo hyperlink
          keyword = "NO"
          nobutton_xpath = CommonObjects.ReplaceString(CommonObjects::ButtonType_xpath,keyword)
...
################################ Test Step  #############################################################
    ...
    Log.TestStep(stepId, "Verify Home page")
      begin
          ...
          #Verify the two news headlines appear under the "Watch List Summary" section in the Homepage.
          if UtilsIphone.VerifyAddedPortFliosTextInHome(stepId, oPage, "Watch List Summary", 2, arr2) == false
             return false
         end
          #Verify the Four news headlines appear under the "Events Summary" section in the Homepage.
          arr = arr1 | arr2
          if UtilsIphone.VerifyAddedPortFliosTextInHome(stepId, oPage, "Events Summary", 4, arr) == false
             return false
         end
   rescue Exception => e
          Log.Error e
...
################################### Test Step  #############################################################
    stepId = 621687
    Log.TestStep(stepId, "SKIP")
      begin
          #Tap on the Skip button
          keyword = 'Skip'
          skipbutton_xpath = CommonObjects.ReplaceString(CommonObjects::ButtonType_xpath,keyword)
          if UtilsMobile.TapElementWithXpath(oPage,skipbutton_xpath,20,true,true,"Skip button ") == false
              return false
          end
          #Verify the Four Portfolios appear under the "Portfolio Contribution Summary" section in the Homepage.
          if UtilsIphone.VerifyAddedPortFliosTextInHome(stepId, oPage, "Portfolio Contribution Summary", 2, arr1) == false
